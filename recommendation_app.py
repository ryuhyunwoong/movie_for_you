...
form_window = uic.loadUiType('./movie_recommendation.ui')[0] #Qt디자인에서 만든ui 불러오는 코드
...
## 클래스 작성
    ...
    def btn_slot(self):
        ...
        if keyword:
            if keyword in self.titles:
                ...
                recommendation = self.recommendation_by_keyword(keyword)    #단어가 없다면 키워드 기반으로 추천
                self.lbl_recommendation.setText(recommendation)
    ...
    def getRecommendation(self, cosine_sim):
        ...
        recMovieList = '\n'.join(recMovieList[1:])  ## 총 11개 중에 첫번째(같은거)빼고 줄바꿈으로 출력
        print(recMovieList)
        return recMovieList
